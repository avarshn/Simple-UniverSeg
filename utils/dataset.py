...
def create_data_loaders(labels_idx, batch_size, num_workers, split):
    ...
    for task_idx in labels_idx:
        d_support = My_OASISDataset(split, task_idx)
...
def create_data_loaders_noshuffle(labels_idx, batch_size, num_workers, split):
    data_loaders = []
    datasets = []
    for task_idx in labels_idx:
        d_support = example_data.My_OASISDataset(split, task_idx)
        supportloader = DataLoader(
            d_support, batch_size=batch_size, shuffle=False, num_workers=num_workers
        )
        data_loaders.append(supportloader)
        datasets.append(d_support)
    return data_loaders, datasets
