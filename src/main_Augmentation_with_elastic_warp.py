...
from utils.utils import seed_everything, create_data_loaders, create_data_loaders_v2
...
def sobel_edge_detection_conv2d(label_tensor):
    ...
    sobel_x = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=label_tensor.dtype, device=label_tensor.device).view(1, 1, 3, 3)
    sobel_y = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=label_tensor.dtype, device=label_tensor.device).view(1, 1, 3, 3)

    reshape_batches = False
    if len(label_tensor.shape) == 5:
        reshape_batches = True
        batch_size = label_tensor.shape[0]
        label_tensor = label_tensor.view(-1, 1, label_tensor.shape[3], label_tensor.shape[4])
        

    # Apply convolutions to compute gradients
    gradient_x = F.conv2d(label_tensor, sobel_x, padding=1, groups=label_tensor.size(1))  # Grouping along channels
    gradient_y = F.conv2d(label_tensor, sobel_y, padding=1, groups=label_tensor.size(1))  # Grouping along channels
    ...
    if reshape_batches:
        binary_gradient = binary_gradient.view(batch_size, -1, 1, binary_gradient.shape[2], binary_gradient.shape[3])
...
def task_augmentation(image, labels, support_images, support_labels):
    ...
    if np.random.rand() < 0.5:
        ...
        shear = 0
        image = transforms.functional.affine(image, degree, [translate_x, translate_y], scale, shear)
        labels = transforms.functional.affine(labels, degree, [translate_x, translate_y], scale, shear)
        support_images = transforms.functional.affine(support_images, degree, [translate_x, translate_y], scale, shear)
        support_labels = transforms.functional.affine(support_labels, degree, [translate_x, translate_y], scale, shear)
    ...
    if np.random.rand() < 0.5:
        ...
        support_images = transforms.functional.adjust_sharpness(support_images, sharpness_factor)

    # Gaussian Blur
    if np.random.rand() < 0.5:
        sigma_val = random.uniform(0.1, 1.1)
        image = transforms.functional.gaussian_blur(image, kernel_size = 5, sigma = sigma_val)
        support_images = transforms.functional.gaussian_blur(support_images, kernel_size = 5, sigma = sigma_val)
...
def main(device: torch.device, writer: SummaryWriter, hyper_parameters : dict):
    ...
    if hyper_parameters["Optimizer"]=="Adam":
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=hyper_parameters["weight_decay"])
    elif hyper_parameters["Optimizer"]=="SGD":
        optimizer = torch.optim.SGD(
        model.parameters(), lr=lr, momentum=hyper_parameters["momentum"], weight_decay=hyper_parameters["weight_decay"])
    ...
    train_data_loaders, train_datasets = create_data_loaders_v2(
        train_labels, batch_size, num_workers, split="support", do_in_task_augmentation = hyper_parameters["do_in_task_augmentation"]
    )
    ...
    for iteration in range(iterations):
        ...
        if hyper_parameters["do_task_augmentation"]:
            image, labels, support_images, support_labels = task_augmentation(image, labels, support_images, support_labels)
        ...
        if iteration % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if over_all_dice > best_dice:
                    ...
                    if not (RESULT_FOLDER / hyper_parameters["experiment_name"]).exists():
                        (RESULT_FOLDER / hyper_parameters["experiment_name"]).mkdir(parents=True)
                    torch.save(
                        model.state_dict(),
                        RESULT_FOLDER
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir=TENSORBOARD_LOG_DIR / hyper_parameters["experiment_name"])
    ...
    print(f"Total time taken to run {hyper_parameters['experiment_name']} experiment - {end_time - start_time}")
